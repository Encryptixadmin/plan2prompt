...
export function buildConversationFindings(
  turns: { question: string; answer: string; turnNumber: number }[]
): string {
  const parts: string[] = [];

  parts.push("=== REFINEMENT INTERVIEW FINDINGS (NEW EVIDENCE FROM THE BUILDER) ===");
  parts.push("");
  parts.push("The builder completed an AI-guided refinement interview. Below is the full Q&A exchange.");
  parts.push("Each question was generated by AI based on the analysis findings and the builder's previous answers.");
  parts.push("You MUST incorporate these findings into your re-analysis. Treat each answer as new evidence.");
  parts.push("Do NOT re-derive information the builder already provided. Do NOT ignore this section.");
  parts.push("");

  parts.push("--- FULL INTERVIEW Q&A TRANSCRIPT ---");
  parts.push("");
  ...
  parts.push("--- ANALYSIS ADJUSTMENT GUIDANCE ---");
  parts.push(`The builder answered ${turns.length} contextual questions during the refinement interview.`);
  parts.push(`- Use the builder's specific answers to ground your updated analysis in their actual situation`);
  parts.push(`- If the builder provided domain-specific details (e.g., target users, competitor names, pricing, technical approaches), reference those details by name in your analysis`);
  parts.push(`- Adjust risk severities and assumption statuses based on the evidence provided`);
  parts.push(`- Feasibility scores should reflect the concrete information now available`);
  parts.push("");
  parts.push("=== END INTERVIEW FINDINGS ===");
